...
namespace Images.Formatters
{
    public class ImageFormatter : BufferedMediaTypeFormatter
    {
        ...
        public override bool CanWriteType(Type type)
        {
            ...
            if (type == typeof(Images.Controllers.BookLinked)) { canWrite = true; }
            // Add more tests if you have other classes that have photos
        ...
        {
            // Use the 'dynamic' language feature to get access to the "Photo" property
            // which we know is inside the 'object' wrapper of the incoming value
            dynamic v = value;

            // Extract the image bytes to an in-memory buffer
            MemoryStream ms;
            if (v.Item.Photo == null)
            {
                ms = new MemoryStream();
            }
            else
            {
                ms = new MemoryStream(v.Item.Photo);
            }
